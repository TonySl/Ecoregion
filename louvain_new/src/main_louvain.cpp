...
long double t_reso= 1L;  // default resolution value Tony
int display_level = -2;
...
usage(char *prog_name, const char *more) {
  ...
  cerr << "-t\tresolutio paramter, larger and larger than 1, less and less clusters will be detected. TonyTony" << endl; //Tony
...
parse_args(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
	...
	break;
	  ...
	  case 't': /////////////////////Tony
	t_reso = atof(argv[i+1]);  //atof not atoi
	i++;
	break;
...
main(int argc, char **argv) {
  ...
  Louvain c(-1, precision, q, t_reso);
  ...
  do {
    ...
    c = Louvain(-1, precision, q, t_reso); // Tony, add reso parameter
