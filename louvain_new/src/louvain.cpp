...
Louvain::Louvain(int nbp, long double epsq, Quality* q, long double reso) {
  qual = q;
  ...
  do {
    ...
    for (int node_tmp = 0 ; node_tmp < qual->size ; node_tmp++) {
      ...
      for (int i=0 ; i<neigh_last ; i++) {
			...
			long double increase = qual->gain(node, neigh_pos[i], neigh_weight[neigh_pos[i]], w_degree, resolution);
