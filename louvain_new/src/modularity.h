...
class Modularity: public Quality {
 public:
  ...
  inline long double gain(int node, int comm, long double dnodecomm, long double w_degree, long double reso2);  //Tony reso2
...
inline long double
Modularity::gain(int node, int comm, long double dnc, long double degc, long double reso2) {  //Tony reso2
  assert(node>=0 && node<size);
  //long double reso3  = 0L;
  long double totc = tot[comm];
  long double m2   = g.total_weight;
  //long double reso  = 31L; // Tony
  return (reso2*dnc - totc*degc/m2);   //Tony reso2
}
